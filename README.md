# 💊 Pill Guy – 스마트 알약 디스펜서 (Team Project)

이 프로젝트는 IoT 기술과 얼굴 인식 알고리즘을 활용해 **정확하고 안전한 약 복용 관리 시스템**을 구현한 3인 팀 프로젝트입니다.  
본인은 **팀장**으로서 프로젝트를 총괄하며, **얼굴 인식 시스템 개발 및 컨트롤러 소프트웨어 구현**을 주도하였습니다.

🔗 원본 팀 프로젝트 레포지토리: [addinedu-ros-8th/iot-repo-1](https://github.com/addinedu-ros-8th/iot-repo-1)
...
| 역할 구분             | 상세 내용 |
|------------------------|-----------|
| 🤖 얼굴 인식 시스템 구현 | - FaceNet512 모델 기반 사용자 얼굴 인식 알고리즘 개발<br> - 인식된 사용자 ID를 기반으로 약 및 스케줄 연동 |
| 🧠 컨트롤러 SW 개발     | - 라즈베리파이 <-> 아두이노 간 통신 프로토콜 설계 (TCP & Serial)<br> - 알약 배분 로직 및 사용자 인터랙션 로직 구현 |
| 📋 팀 리딩 및 통합 관리 | - 전체 시스템 구조 설계 및 아키텍처 정의<br> - 하드웨어 구성 설계 및 통합 테스트 주도 |
...
| 이름     | 역할 |
|----------|------|
| **이명운** | 팀장, 얼굴 인식 시스템 및 컨트롤러 SW 구현 |
| 남상기   | 펌웨어 SW 개발, 아두이노 기반 하드웨어 회로 구성 |
| 박세린   | GUI 인터페이스 구현 (PyQt5), DB 설계 및 쿼리 로직 개발 |
